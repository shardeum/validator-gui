...
const Login = ({ apiPort }: any) => {
  function handleLogin(e: any) {
    useAuth(apiPort, e.target.password.value)
  }
    ...
    <>
      <div className="bg-white text-stone-500	rounded-xl p-8 text-sm [&>*]:pb-2 max-w-2xl">
        ...
        <form onSubmit={(e: any) => handleLogin(e)}>
          ...
          ></input>
          <button className="p-4 bg-blue-700 text-stone-200" type="submit">
            Connect
          </button>
        </form>
      </div>
...
Login.getLayout = function getLayout(page: ReactElement) {
  return <div className="p-5 flex justify-center">{page}</div>
}
