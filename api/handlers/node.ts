...
export default function configureNodeHandlers(apiRouter: Router) {
  ...
  //Consider extending NodeStatusResponse to include an 'error' property for enhanced error handling
  apiRouter.get(
    "/node/status",
    asyncRouteHandler(
      async (req: Request, res: Response<NodeStatusResponse>) => {
        // First, execute operator-cli status to get the current status
        execFile("operator-cli", ["status"], (err, stdout, stderr) => {
          ...
          // Check if lockedStake is less than 10
          if (
            yamlData.state === "standby" &&
            parseFloat(yamlData.lockedStake) < 10
          ) {
            // If so, call operator-cli stop to stop the node
            execFileSync("operator-cli", ["stop", "-f"]);
            // Update the state to reflect the change after stopping the node
            yamlData.state = "stopped";
            console.log("Node stopped due to insufficient lockedStake.");
          }

          // Continue with previous logic for handling 'active' and 'stopped' states
  ...
  apiRouter.get(
    ...
    asyncRouteHandler(
      async (req: Request, res: Response<NodeStatusResponse>) => {
        ...
        console.log("executing operator-cli status...");
        const output = execFileSync("operator-cli", ["stake_info", address], {
          encoding: "utf8",
        });
  ...
  apiRouter.post(
    "/node/status/history",
    asyncRouteHandler(
      async (req: Request, res: Response<NodeStatusHistoryResponse>) => {
  ...
  apiRouter.get(
    ...
    asyncRouteHandler(
      async (req: Request, res: Response<NodeVersionResponse>) => {
        ...
        const output = execFileSync("operator-cli", ["version"], {
          encoding: "utf8",
        });
  ...
  const getSettings = () => {
    const output = execFileSync("operator-cli", ["node-settings"], {
      encoding: "utf8",
    });
